...
def _format_rhs(definition):
    ...
    if klass == 'ConstantExpressionNode':
        ...
        if not all( isinstance(i, dict) for i in (left, right) ):
            _logger.warn('Unknown BinaryExpressionNode type(left)=%r type(right)=%r',
...
def _convert_enum(definition, f_out, indent_level):
    ...
    if not name or not isinstance(fields, (list, NoneType)):
        _logger.warn('Unknown enum name=%r type(fields)=%r', name, type(fields))
        return False
...
def _convert_function(definition, f_out, indent_level):
    ...
    _put(f_out, indent_level, 'cdef ', *returns_args, ' ', name, '(', *args_args, ')')
    return True
...
def _convert_macro(definition, f_out, indent_level):
    ...
    if name == value:
        ...
        if m:
            ...
            value = int(digits, 10 if not prefix else 16 if prefix == '0x' else 0)
            if minus:
                value = -1
...
def _convert_struct(definition, f_out, indent_level, struct='struct'):
    ...
    fields = definition.get('fields')
    if not name or not isinstance(fields, (list, NoneType)):
        _logger.warn('Unknown %s data name=%r type(ctype)=%r', struct, name, type(fields))
        return False
    ...
    if fields is None:
        _put(f_out, indent_level,
            'cdef ', struct, ' ', *name_args,
            '  # forward declaration'
        )
    else:
        _put(f_out, indent_level, 'cdef ', struct, ' ', *name_args, ':')
        for field in fields:
            _convert_typedef(field, f_out, indent_level + 1, include_cdef=False)
...
def _convert_base_Klass(base):
    ...
    if klass == 'CtypesSimple':
        ...
        if not tag or not variety:
            _logger.warn('Unsupported base Klass=%r tag=%r variety=%r', klass, tag, variety)
            return False
        ...
        if not isinstance(destination, dict):
            _logger.warn('Unsupported base Klass type(destination)=%r', destination)
            return False
...
def _convert_typedef_CtypesArray(name, ctype, include_cdef):
    ...
    if not all( isinstance(i, dict) for i in (base, count) ):
        _logger.warn('CtypesArray type(base)=%r type(count)=%r', type(base), type(count))
        return False
...
def _convert_typedef(definition, f_out, indent_level, include_cdef=True):
    ...
    if not name or not isinstance(ctype, dict):
        _logger.warn('Unknown typedef data name=%r type(ctype)=%r', name, type(ctype))
        return False
...
def convert(f_in, f_out, import_from='*', indent_level=0):
    ...
    _put(f_out, indent_level, 'cdef extern from ', import_from or '*', ' nogil:')
    indent_level += 1

    for definition in load(f_in):
